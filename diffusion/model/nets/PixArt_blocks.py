...
class TokenCrossStreamConv(nn.Module):
    def __init__(self, hidden_size: int):
        ...
        self.dw = zero_module(
            nn.Conv2d(hidden_size, hidden_size, kernel_size=3, stride=1, padding=1, groups=hidden_size)
        )
        self.pw = nn.Conv2d(hidden_size, hidden_size, kernel_size=1, stride=1, padding=0)
        nn.init.xavier_uniform_(self.pw.weight)
        nn.init.zeros_(self.pw.bias)

    def forward(self, x: torch.Tensor, lr_tokens: torch.Tensor, HW):
        if x is None or lr_tokens is None:
        ...
        if h * w != n:
            raise RuntimeError(f"TokenCrossStreamConv requires H*W == N, got H={h}, W={w}, N={n}")
        if lr_tokens.shape[1] != n:
            raise RuntimeError(f"lr_tokens length mismatch: got {lr_tokens.shape[1]}, expected {n}")

        lr_f = self.lr_norm(lr_tokens).reshape(b, h, w, c).permute(0, 3, 1, 2).contiguous()
        x_f = self.x_norm(x).reshape(b, h, w, c).permute(0, 3, 1, 2).contiguous()
        f = lr_f - x_f
        delta = self.pw(self.dw(f))
        return delta.permute(0, 2, 3, 1).reshape(b, n, c).contiguous()

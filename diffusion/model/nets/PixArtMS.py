        ...
        **kwargs,
        ...
        if (not self.has_semantic_adapter) or (semantic_tokens is None):
            ...
            if semantic_gate is not None:
                gate = img_gate * semantic_gate.to(dtype=x.dtype)
            ...
            self._last_semantic_stats = {
                ...
                "semantic_gate_mean": float(torch.as_tensor(gate if gate is not None else img_gate, dtype=x.dtype, device=x.device).mean().item()),
                "hpa_text_out_std": float(hpa_stats.get("text_out_std", 0.0)),
                "hpa_img_out_std": float(hpa_stats.get("img_out_std", 0.0)),

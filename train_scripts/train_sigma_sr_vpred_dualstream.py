...
OUT_DIR = os.path.join(OUT_BASE, "pixart_dualcondsr_dit4sr_pisa_v1")
...
USE_ADAPTIVE_TEXT_PROMPT = bool(int(os.getenv("USE_ADAPTIVE_TEXT_PROMPT", "0")))
...
PIXEL_LAYERS = list(range(0, 16))
LR_CONV_LAYERS = list(range(0, 8))
SEMANTIC_LAYERS = list(range(20, 28))
ANCHOR_LAYERS = PIXEL_LAYERS
...
def sample_t(batch: int, device: str, step: int) -> torch.Tensor:
    ...
    if mode == 'two_stage':
        ...
        return torch.randint(tmin, tmax + 1, (batch,), device=device).long()
    if mode == "mixture":
        choose_high = torch.rand((batch,), device=device) < float(T_MIX_HIGH_PROB)
        t = torch.empty((batch,), device=device, dtype=torch.long)
        nh = int(choose_high.sum().item())
        if nh > 0:
            t[choose_high] = torch.randint(int(T_MIX_HIGH_MIN), tmax + 1, (nh,), device=device)
        nl = int((~choose_high).sum().item())
        if nl > 0:
            low_max = min(int(T_MIX_LOW_MAX), tmax)
            u = torch.rand((nl,), device=device)
            span = float(max(1, low_max - tmin))
            t_low = torch.floor((u ** float(T_MIX_LOW_POWER)) * span + tmin).clamp(tmin, low_max).long()
            t[~choose_high] = t_low
        return t
...
def merge_prompt_packs_or_null(prompt_packs, null_pack, batch_size, device):
    ys, masks = [], []
    for p in prompt_packs:
        if p is None:
            ys.append(null_pack["y"])
            masks.append(null_pack["mask"])
        else:
            ys.append(p["y"])
            masks.append(p["mask"])
    y = torch.cat(ys, dim=0).to(device)
    m = torch.cat(masks, dim=0).to(device)
    return y[:batch_size], m[:batch_size]
...
class MixedRealISRDataset(Dataset):
    """
    Mixed dataset: paired DF2K/RealSR + synthetic degradation branches.
    For this first version, synthetic branches reuse DegradationPipeline and emit source_type/pixel_loss_weight.
    """
    TRAIN_HR_SYN_ROOTS = [TRAIN_DF2K_HR_DIR, os.getenv("TRAIN_OST_HR_DIR", ""), os.getenv("TRAIN_LSDIR_HR_DIR", "")]
    MIX_SYN_REALESRGAN_PROB = 0.40
    MIX_SYN_BSRGAN_PROB = 0.25
    MIX_PAIRED_DF2K_PROB = 0.25
    MIX_PAIRED_REALSR_PROB = 0.10
        ...
        for root in TRAIN_REALSR_DIRS:
            for hr_path in sorted(glob.glob(os.path.join(root, "*_HR.png"))):
                ...
                if os.path.exists(lr_path):
                    self.realsr.append((hr_path, lr_path))
        self.syn_hr_paths = []
        for rr in self.TRAIN_HR_SYN_ROOTS:
            if rr and os.path.isdir(rr):
                self.syn_hr_paths.extend(_scan_images(rr))
        self.syn_hr_paths = [str(p) for p in self.syn_hr_paths]
        self.pipeline = DegradationPipeline(crop_size=crop_size)
        probs = np.array([
            self.MIX_SYN_REALESRGAN_PROB, self.MIX_SYN_BSRGAN_PROB,
            self.MIX_PAIRED_DF2K_PROB, self.MIX_PAIRED_REALSR_PROB
        ], dtype=np.float64)
        probs = probs / probs.sum()
        self.mix_probs = probs.tolist()
        self.to_tensor = transforms.ToTensor()
        self.norm = transforms.Normalize(mean=[0.5] * 3, std=[0.5] * 3)

    def __len__(self):
        return max(len(self.df2k), len(self.syn_hr_paths), 1)

    def _sample_syn(self, mode: str):
        if len(self.syn_hr_paths) == 0:
            return self.df2k[random.randint(0, len(self.df2k) - 1)]
        hp = random.choice(self.syn_hr_paths)
        hr_pil = Image.open(hp).convert("RGB")
        hr_crop = random_crop_pil(hr_pil, self.crop_size, self.crop_size)
        hr = self.norm(self.to_tensor(hr_crop))
        lr = self.pipeline(hr, return_meta=False)
        lr_small = F.interpolate(lr.unsqueeze(0), size=(self.crop_size // 4, self.crop_size // 4), mode="bicubic", align_corners=False, antialias=True).squeeze(0).clamp(-1.0, 1.0)
        return {
            "hr": hr, "lr": lr, "lr_small": lr_small, "path": hp, "sample_key": make_sample_key(hp),
            "source_type": mode, "pixel_loss_weight": 1.0
        }

    def __getitem__(self, idx):
        choice = int(np.random.choice(4, p=self.mix_probs))
        if choice == 0:
            return self._sample_syn("syn_realesrgan")
        if choice == 1:
            return self._sample_syn("syn_bsrgan")
        if choice == 2:
            it = self.df2k[idx % len(self.df2k)]
            it["source_type"] = "paired_df2k"; it["pixel_loss_weight"] = 0.8
            return it
        if len(self.realsr) == 0:
            it = self.df2k[idx % len(self.df2k)]
            it["source_type"] = "paired_df2k"; it["pixel_loss_weight"] = 0.8
            return it
        hr_path, lr_path = self.realsr[idx % len(self.realsr)]
        hr = self.norm(self.to_tensor(Image.open(hr_path).convert("RGB")))
        lr_small_pil = Image.open(lr_path).convert("RGB")
        lr_small = self.norm(self.to_tensor(TF.resize(lr_small_pil, (self.crop_size // 4, self.crop_size // 4), interpolation=transforms.InterpolationMode.BICUBIC, antialias=True)))
        lr = self.norm(self.to_tensor(TF.resize(lr_small_pil, (self.crop_size, self.crop_size), interpolation=transforms.InterpolationMode.BICUBIC, antialias=True)))
        return {
            "hr": hr, "lr": lr, "lr_small": lr_small, "path": hr_path, "sample_key": make_sample_key(hr_path),
            "source_type": "paired_realsr", "pixel_loss_weight": 0.5
        }
...
PIXEL_LORA_RANK = 16
PIXEL_LORA_ALPHA = 16
SEMANTIC_LORA_RANK = 16
SEMANTIC_LORA_ALPHA = 16


class DualLoRALinear(nn.Module):
    def __init__(self, base: nn.Linear, pixel_rank: int, semantic_rank: int, pixel_alpha: float, semantic_alpha: float):
        super().__init__()
        self.base = base
        self.pixel_scaling = float(pixel_alpha) / float(pixel_rank)
        self.semantic_scaling = float(semantic_alpha) / float(semantic_rank)
        self.pixel_lora_A = nn.Linear(base.in_features, pixel_rank, bias=False, dtype=torch.float32)
        self.pixel_lora_B = nn.Linear(pixel_rank, base.out_features, bias=False, dtype=torch.float32)
        self.semantic_lora_A = nn.Linear(base.in_features, semantic_rank, bias=False, dtype=torch.float32)
        self.semantic_lora_B = nn.Linear(semantic_rank, base.out_features, bias=False, dtype=torch.float32)
        self.lambda_pix = 1.0
        self.lambda_sem = 1.0
    ...
    def set_lora_scales(self, lambda_pix: float = 1.0, lambda_sem: float = 1.0):
        self.lambda_pix = float(lambda_pix)
        self.lambda_sem = float(lambda_sem)

    def forward(self, x):
        out = self.base(x)
        x32 = x.float()
        pix = self.pixel_lora_B(self.pixel_lora_A(x32)) * self.pixel_scaling
        sem = self.semantic_lora_B(self.semantic_lora_A(x32)) * self.semantic_scaling
        return out + self.lambda_pix * pix.to(out.dtype) + self.lambda_sem * sem.to(out.dtype)
...
def set_dual_lora_scales(model, lambda_pix: float = 1.0, lambda_sem: float = 1.0):
    for m in model.modules():
        if hasattr(m, "set_lora_scales"):
            m.set_lora_scales(lambda_pix=lambda_pix, lambda_sem=lambda_sem)


def apply_dual_lora(model):
    cnt = 0
    for name, module in list(model.named_modules()):
        ...
        setattr(
            ...
            DualLoRALinear(
                module,
                pixel_rank=int(PIXEL_LORA_RANK),
                semantic_rank=int(SEMANTIC_LORA_RANK),
                pixel_alpha=float(PIXEL_LORA_ALPHA),
                semantic_alpha=float(SEMANTIC_LORA_ALPHA),
            ),
        )
        cnt += 1
    print(f"✅ Dual LoRA applied to {cnt} layers (pixel_rank={PIXEL_LORA_RANK}, semantic_rank={SEMANTIC_LORA_RANK}).")


def configure_pixart_trainable_params(pixart: nn.Module, train_x_embedder: bool = False):
    pixel_set = set(PIXEL_LAYERS)
    lr_conv_set = set(LR_CONV_LAYERS)
    sem_set = set(SEMANTIC_LAYERS)
    ...
    for name, p in pixart.named_parameters():
        ...
        if (
            ...
            or name.startswith("local_entry_norm.")
            or name.startswith("local_entry_proj.")
            or name == "local_entry_gate"
        ):
            p.requires_grad = True
            continue
        if (
            name.startswith("lr_ip_norm.")
            or name.startswith("lr_ip_proj.")
            or name.startswith("sem_ip_norm.")
            or name.startswith("sem_ip_proj.")
        ):
            p.requires_grad = True
            continue
        if bid is not None and bid in pixel_set and (
            ".lr_ip_attn." in name
            or ".lr_stream_attn." in name
            or name.endswith(".lr_ip_gate")
            or name.endswith(".lr_stream_gate")
        ):
            p.requires_grad = True
            continue
        if bid is not None and bid in lr_conv_set and (
            ".lr_cross_conv." in name or name.endswith(".lr_cross_conv_gate")
        ):
            p.requires_grad = True
            continue
        if bid is not None and bid in sem_set and (
            name.endswith(".semantic_gate")
            or ".cross_attn.k_img_linear." in name
            or ".cross_attn.v_img_linear." in name
            or ".cross_attn.img_residual_linear." in name
        ):
            p.requires_grad = True
            continue
        if bid is not None:
            if ("pixel_lora_A" in name or "pixel_lora_B" in name):
                p.requires_grad = bid in pixel_set
                continue
            if ("semantic_lora_A" in name or "semantic_lora_B" in name):
                p.requires_grad = bid in sem_set
                continue
    ...
    required_substrings = [
        "lr_ip_proj", "lr_ip_attn", "lr_stream_attn", "lr_cross_conv", "sem_ip_proj",
        "cross_attn.k_img_linear", "cross_attn.v_img_linear", "cross_attn.img_residual_linear",
        "pixel_lora", "semantic_lora", "final_layer", "local_entry"
    ]
    for s in required_substrings:
        if not any(s in n for n in trainable_names):
            raise RuntimeError(f"Required trainable path missing: {s}")
...
def collect_sem_adapter_trainable_state_dict(sem_adapter: nn.Module):
    keep = {}
    for n, p in sem_adapter.state_dict().items():
        if n.startswith("resampler.") or n.startswith("proj_norm.") or n.startswith("proj.") or n == "out_scale":
            keep[n] = p.detach().float().cpu()
    return keep
...
def build_optimizer_and_clippables(pixart: nn.Module, adapter: nn.Module, sem_adapter: nn.Module):
    ...
    pixart_condition_params, final_layer_params = [], []
    pixel_lora_params, semantic_lora_params = [], []
    for n, p in pixart.named_parameters():
        if not p.requires_grad:
            continue
        if n.startswith("final_layer."):
            final_layer_params.append(p)
        elif ("pixel_lora_A" in n) or ("pixel_lora_B" in n):
            pixel_lora_params.append(p)
        elif ("semantic_lora_A" in n) or ("semantic_lora_B" in n):
            semantic_lora_params.append(p)
        else:
            pixart_condition_params.append(p)
    sem_adapter_params = [
        p for n, p in sem_adapter.named_parameters()
        if p.requires_grad and (("resampler" in n) or ("proj_norm" in n) or ("proj." in n) or ("out_scale" in n))
    ]

    optim_groups = []
    if adapter_params:
        optim_groups.append({"params": adapter_params, "lr": 3e-4, "weight_decay": 0.01})
    if pixart_condition_params:
        optim_groups.append({"params": pixart_condition_params, "lr": 1e-4, "weight_decay": 0.01})
    if final_layer_params:
        optim_groups.append({"params": final_layer_params, "lr": 5e-5, "weight_decay": 0.01})
    if pixel_lora_params:
        optim_groups.append({"params": pixel_lora_params, "lr": 1e-4, "weight_decay": 0.01})
    if semantic_lora_params:
        optim_groups.append({"params": semantic_lora_params, "lr": 1e-4, "weight_decay": 0.01})
    ...
    params_to_clip = adapter_params + pixart_condition_params + final_layer_params + pixel_lora_params + semantic_lora_params + sem_adapter_params

    pixart_trainable = [p for p in pixart.parameters() if p.requires_grad]
    grouped_pixart = pixart_condition_params + final_layer_params + pixel_lora_params + semantic_lora_params
    ...
    all_group_params = adapter_params + grouped_pixart + sem_adapter_params
    if len({id(p) for p in all_group_params}) != len(all_group_params):
        raise RuntimeError("Optimizer groups overlap across adapter/pixart/sem_adapter.")

    group_counts = {
        "adapter": len(adapter_params),
        "pixart_condition": len(pixart_condition_params),
        "final_layer": len(final_layer_params),
        "pixel_lora": len(pixel_lora_params),
        "semantic_lora": len(semantic_lora_params),
...
def save_smart(
    ...
    def _build_resume_state_snapshot():
        ...
        adapter_token_head_counts = {
            "adapter.ip_token_head": int(sum(1 for k in adapter_sd.keys() if k.startswith("ip_token_head."))),
            "adapter.local_entry_head": int(sum(1 for k in adapter_sd.keys() if k.startswith("local_entry_head."))),
            "adapter.fuse64": int(sum(1 for k in adapter_sd.keys() if k.startswith("fuse64."))),
        }
        ...
        "sem_adapter": sem_adapter_sd,
            ...
            "layer_config": {
                ...
                "dual_lora": True,
                "pixel_lora_rank": int(PIXEL_LORA_RANK),
                "pixel_lora_alpha": float(PIXEL_LORA_ALPHA),
                "semantic_lora_rank": int(SEMANTIC_LORA_RANK),
                "semantic_lora_alpha": float(SEMANTIC_LORA_ALPHA),
                "use_semantic_branch": bool(USE_SEMANTIC_BRANCH),
            ...
            for t in run_timesteps:
                ...
                with torch.autocast(device_type="cuda", dtype=COMPUTE_DTYPE):
                    ...
                    if USE_SEMANTIC_BRANCH:
                        lr_small_01 = (lr_small.to(dtype=torch.float32).clamp(-1, 1) + 1.0) * 0.5
                        sem_tokens = sem_adapter(lr_small_01)
                        cond = dict(cond)
                        cond["sem_tokens"] = sem_tokens.to(dtype=COMPUTE_DTYPE)
                with torch.autocast(device_type="cuda", dtype=COMPUTE_DTYPE):
                    ...
                    if USE_ADAPTIVE_TEXT_PROMPT and ADAPTIVE_PROMPT_CACHE_ROOT:
                        ...
                        y_cond, y_mask_cond = merge_prompt_packs_or_null(prompt_packs, null_pack, latents.shape[0], DEVICE)
...
def main():
    ...
    pixart.enable_sr_conditioning_layers(
        ...
        lr_conv_layers=LR_CONV_LAYERS,
        semantic_layers=SEMANTIC_LAYERS,
        init_gate=-4.0,
    )
    if ENABLE_LORA:
        apply_dual_lora(pixart)
    else:
    ...
    for epoch in range(ep_start, 1000):
        ...
        for i, batch in enumerate(pbar):
            ...
            with torch.autocast(device_type="cuda", dtype=COMPUTE_DTYPE):
                ...
                if USE_SEMANTIC_BRANCH:
                    lr_small_01 = (lr_small_b.to(DEVICE, dtype=torch.float32).clamp(-1, 1) + 1.0) * 0.5
                    sem_tokens = sem_adapter(lr_small_01)
                    cond = dict(cond)
                    cond["sem_tokens"] = sem_tokens.to(dtype=COMPUTE_DTYPE)
            ...
            with torch.autocast(device_type="cuda", dtype=COMPUTE_DTYPE):
                ...
                if USE_ADAPTIVE_TEXT_PROMPT and ADAPTIVE_PROMPT_CACHE_ROOT:
                    ...
                    if not all(p is not None for p in prompt_packs):
                        ...
                        y_cond, y_mask = merge_prompt_packs_or_null(prompt_packs, null_pack, zt.shape[0], DEVICE)
                ...
                image_cond_stats = getattr(pixart, "_last_image_stats", {}) or {}
            ...
            if i % 10 == 0:
                ...
                log_dict = {
                    ...
                    'ip_tok_std': f"{ip_tokens_std:.4f}",
                    'loc_tok_std': f"{local_entry_tokens_std:.4f}",
                ...
                LAST_TRAIN_LOG = {
                    ...
                    "ip_tokens_std": ip_tokens_std,
                    "local_entry_tokens_std": local_entry_tokens_std,

...
def build_model_and_assets(args, device, compute_dtype):
    ...
    pixart = PixArtSigmaSR_XL_2(
        ...
        in_channels=4,
        out_channels=4,
        pixel_layers=list(layer_cfg.get("pixel_layers", layer_cfg.get("anchor_layers", list(range(0, 16))))),
        lr_conv_layers=list(layer_cfg.get("lr_conv_layers", list(range(0, 8)))),
        anchor_layers=list(layer_cfg.get("pixel_layers", layer_cfg.get("anchor_layers", list(range(0, 16))))),
        semantic_layers=list(layer_cfg.get("semantic_layers", list(range(20, 28)))),
    ).to(device)
    ...
    pixart.enable_sr_conditioning_layers(
        pixel_layers=layer_cfg.get("pixel_layers", layer_cfg.get("anchor_layers", list(range(0, 16)))),
        lr_conv_layers=layer_cfg.get("lr_conv_layers", list(range(0, 8))),
        semantic_layers=layer_cfg.get("semantic_layers", list(range(20, 28))),
        init_gate=-4.0,
    )
    if has_lora:
    ...
    sem_adapter = None
    if not args.disable_semantic_branch:
        sem_enc = args.semantic_encoder_name_or_path or os.getenv("SEMANTIC_ENCODER_NAME_OR_PATH", "openai/clip-vit-large-patch14")
        sem_adapter = CLIPSemanticAdapter(encoder_name_or_path=sem_enc, hidden_size=1152, num_prompt_tokens=16, clip_input_res=224).to(device).eval()
        sem_sd = ckpt.get("sem_adapter", None)
        if isinstance(sem_sd, dict):
            sem_adapter.load_state_dict(sem_sd, strict=False)
...
def run_ddim_predict(pixart, adapter, sem_adapter, vae, null_pack, scheduler, batch, args, device, compute_dtype, gen, prompt_cache_mem):
    ...
    for t in run_timesteps:
        ...
        with torch.autocast(device_type="cuda", dtype=compute_dtype, enabled=(device == "cuda")):
            ...
            if sem_adapter is not None:
                lr_small_01 = (lr_small.to(device=device, dtype=torch.float32).clamp(-1, 1) + 1.0) * 0.5
                cond = dict(cond)
                cond["sem_tokens"] = sem_adapter(lr_small_01).to(dtype=compute_dtype)
...
def parse_args():
    ...
    parser.add_argument("--semantic-encoder-name-or-path", type=str, default=os.getenv("SEMANTIC_ENCODER_NAME_OR_PATH", "openai/clip-vit-large-patch14"))

...
from diffusion.model.nets.unet_discriminator_sn import UNetDiscriminatorSN
from diffusion.model.losses.perceptual_loss import PerceptualLoss
...
def get_config_snapshot():
    return {
        ...
        "loss_weights": "Lv+0.10L1+0.05LRcons+0.05GW+Percep+GAN",
...
def parse_stage2_args():
    parser = argparse.ArgumentParser(description="Stage-2 perceptual + GAN fine-tune for SFT+GW SR")
...
def color_anchor_loss(pred_m11: torch.Tensor, hr_m11: torch.Tensor) -> torch.Tensor:
    # Engineering safeguard only; not a paper-core contribution.
    pred_low = F.avg_pool2d(pred_m11.float(), kernel_size=11, stride=1, padding=5)
    hr_low = F.avg_pool2d(hr_m11.float(), kernel_size=11, stride=1, padding=5)
    return F.l1_loss(pred_low, hr_low)


def gan_loss(logits: torch.Tensor, target_is_real: bool) -> torch.Tensor:
...
def main():
    ...
    adapter = build_adapter_v7(in_channels=3, hidden_size=1152, injection_layers_map=getattr(pixart, "injection_layer_to_level", getattr(pixart, "injection_layers", None))).to(DEVICE).train()
    ...
    if not init_from_ckpt_weights_only(pixart, adapter, stage2_args.stage2_ckpt):
        raise FileNotFoundError(f"Stage-2 checkpoint not found or failed to load: {stage2_args.stage2_ckpt}")
    ...
    for epoch in range(ep_start, 1000):
        ...
        for i, batch in enumerate(pbar):
            ...
            with torch.autocast(device_type="cuda", dtype=COMPUTE_DTYPE):
                ...
                loss_color_anchor = torch.tensor(0.0, device=DEVICE)

                need_pixel_loss = True
                ...
                if calc_pixel_loss:
                    ...
                    loss_percep = percep_loss_fn(img_p_valid, img_t_valid)
